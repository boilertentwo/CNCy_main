...
export async function Servemodelimage({folder, tag}){
  const asset = folder + tag
  try {
    const resources = await cloudinary.search
        .expression(asset)
        .sort_by('public_id')
        .execute()
    return <Modelimages images={resources.resources}/>
  } catch (error) {
      console.log("error fetching images", error)
      return <div>Check your internet connections</div>
  }
}